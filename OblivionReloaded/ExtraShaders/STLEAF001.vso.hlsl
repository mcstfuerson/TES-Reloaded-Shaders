...
#define	sqr(v)			((v) * (v))
    ...
    q9.xy = (((((q7.x * ((q7.x * q8.x) - 0.5)) + 1) * RustleParams.z) * RustleParams.x) / (PI * 2)) + const_12.yz;
    r1.xy = angler(frac(q9.xy)) * angler(frac(q9.xy));
    q3.xy = (((((q1.x * ((q1.x * q2.x) - 0.5)) + 1) * RockParams.z) * RockParams.x) / (PI * 2)) + const_12.yz;
    ...
    q13.xyz = normalize((LeafLighting.y * (r2.xyz / length(r2.xyzw))) + IN.normal.xyz);
    r0.y = -r0.w;
    ...
    q146.xyzw = ((r4.x * r0.xyzw) + (r5.x * r1.xyzw)) + IN.position.xyzw;
    q27.xyzw = mul(float4x4(WindMatrices[0 + q12.x].xyzw, WindMatrices[1 + q12.x].xyzw, WindMatrices[2 + q12.x].xyzw, WindMatrices[3 + q12.x].xyzw), q146.xyzw);
    r0.xyzw = (SunDimmer.x * (shades(q13.xyz, LightVector.xyz) * DiffColor.rgba)) + AmbientColor.rgba;
    q58.xyzw = (IN.blendindices.x * (q27.xyzw - q146.xyzw)) + q146.xyzw;
    mdl59.xyz = mul(float3x4(ModelViewProj[0].xyzw, ModelViewProj[1].xyzw, ModelViewProj[2].xyzw), q58.xyzw);
    OUT.position.w = dot(ModelViewProj[3].xyzw, q58.xyzw);
    OUT.position.xyz = mdl59.xyz;
    OUT.texcoord_0.xy = IN.texcoord_0.xy;
    OUT.texcoord_1.w = r0.w;
    OUT.texcoord_1.xyz = r0.xyz * frac(IN.blendindices.z);
    OUT.texcoord_2.w = 1 - saturate((FogParam.x - length(mdl59.xyz)) / FogParam.y);
