...
#define	sqr(v)			((v) * (v))
    ...
    OUT.Light0Dir.xyz = normalize(r3.xyz);
    OUT.Light1Dir.xyz = mul(float3x3(q29.xyz, q41.xyz, q14.xyz), normalize(lit3.xyz));
    OUT.Att1UV.w = 0.5;
    OUT.Att1UV.xyz = compress(lit3.xyz / LightPosition[1].w);	// [-1,+1] to [0,1]
    OUT.ShadowUV.xy = ((q122.x * ShadowProjTransform.xy) + m147.xy) / (q122.x * ShadowProjTransform.w);
    OUT.ShadowUV.zw = ((m147.xy - ShadowProjData.xy) / ShadowProjData.w) * float2(1, -1) + float2(0, 1);
    OUT.CameraDir.xyz = normalize(eye121.xyz);
