...
#define	sqr(v)			((v) * (v))
    ...
    OUT.texcoord_1.xyz = normalize(m95.xyz);
    OUT.texcoord_2.xyz = mul(float3x3(q33.xyz, q39.xyz, q45.xyz), normalize(lit3.xyz));
    OUT.texcoord_3.xyz = mul(float3x3(q33.xyz, q39.xyz, q45.xyz), normalize(lit5.xyz));
    OUT.texcoord_4.w = 0.5;
    OUT.texcoord_4.xyz = compress(lit3.xyz / LightPosition[1].w);
    // [-1,+1] to [0,1]
    OUT.texcoord_5.w = 0.5;
    OUT.texcoord_5.xyz = compress(lit5.xyz / LightPosition[2].w);
    // [-1,+1] to [0,1]
    OUT.texcoord_7.xyz = normalize(eye129.xyz);
    OUT.texcoord_8 = mul(OUT.position, TESR_ShadowCameraToLightTransform[0]);
    OUT.texcoord_6 = mul(OUT.position, TESR_ShadowCameraToLightTransformSkin);
    OUT.texcoord_9 = mul(OUT.position, TESR_InvViewProjectionTransform);
