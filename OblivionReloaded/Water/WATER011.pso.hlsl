...
//
//   vsa shaderdump19/WATER011.pso /Fcshaderdump19/WATER011.pso.dis
//
//
// Parameters:
//
...
float3 getWaterNorm( float2 tex, float dist, float camera_vector_z, inout float3 specNorm, inout float lod, inout float height )
{
	...
	lod = max( saturate( (camera_vector_z*camera_vector_z) * 50 * TESR_ProjectionTransform[0][0] /(TESR_ReciprocalResolution.x * dist) * LODdistance ), MinLOD);
...
PS_OUTPUT main(VS_OUTPUT IN, float2 PixelPos : VPOS) {
	...
	float lod = 0;
	float height = 0;
	normal = getWaterNorm( surfPos, depth - uw_pos, -camera_vector.z, specNorm, lod, height);
	eyepos.z += height;
	...
	if (lava)
	{
			water_result.rgb = tex2D(DetailMap, IN.texcoord_6.xy + Scroll.xy).rgb;
			water_result = lerp( float4(1,0.33,0.1,1), water_result, lod );
			water_result.r += 0.1;
	}
