// GodRays full screen shader for Oblivion/Skyrim Reloaded, adapted for Secunda moon
...
float4 TESR_SecundaRaysRay;
float4 TESR_SecundaRaysRayColor;
float4 TESR_SecundaRaysData;

sampler2D TESR_RenderedBuffer : register(s0) = sampler_state { ADDRESSU = CLAMP; ADDRESSV = CLAMP; MAGFILTER = LINEAR; MINFILTER = LINEAR; MIPFILTER = LINEAR; };
sampler2D TESR_DepthBuffer : register(s1) = sampler_state { ADDRESSU = CLAMP; ADDRESSV = CLAMP; MAGFILTER = LINEAR; MINFILTER = LINEAR; MIPFILTER = LINEAR; };
sampler2D TESR_SourceBuffer : register(s2) = sampler_state { ADDRESSU = CLAMP; ADDRESSV = CLAMP; MAGFILTER = LINEAR; MINFILTER = LINEAR; MIPFILTER = LINEAR; };

static const float4 sp = TESR_SecundaDirection * 999999;
...
static const float forward = dot(-TESR_SecundaDirection, TESR_CameraForward);
static const int ShaftPasses = int(TESR_SecundaRaysData.x);
...
float4 DepthSkyMask(VSOUT IN) : COLOR0
{
	...
	if (forward < 0.0f) {
		...
		sky = tex2D(TESR_SourceBuffer, IN.UVCoord).rgb * depth * TESR_SecundaRaysData.y;
	}
...
float4 LightShaft(VSOUT IN) : COLOR0
{
	...
	DeltaTexCoord *= 0.5f * min(0.3f, screendist) * (1.0f / TESR_SecundaRaysData.x) * TESR_SecundaRaysRay.z;
	...
	for (int i = 0; i < ShaftPasses; i++)
	{
		...
		Sample *= IlluminationDecay * TESR_SecundaRaysRay.w;
		Color += Sample;
		IlluminationDecay *= TESR_SecundaRaysRay.y;
	}
	Color *= TESR_SecundaRaysRay.x / TESR_SecundaRaysData.x;
	return float4(Color, 1.0f);
}

float4 Blur(float2 Tex : TEXCOORD0) : COLOR0
{
	float4 col = tex2D(TESR_RenderedBuffer, Tex);
	float2 tangent = normalize(Tex - sunview).yx * float2(TESR_ReciprocalResolution.y, -TESR_ReciprocalResolution.x);
	col += 0.67f * tex2D(TESR_RenderedBuffer, Tex + tangent);
	col += 0.67f * tex2D(TESR_RenderedBuffer, Tex - tangent);
	col += 0.33f * tex2D(TESR_RenderedBuffer, Tex + 2.0f * tangent);
	col += 0.33f * tex2D(TESR_RenderedBuffer, Tex - 2.0f * tangent);
	if (forward > 0.0f) col = 0.0f;
...
{
	float Amount = saturate(TESR_SecundaAmount.x);
	float4 ori = tex2D(TESR_SourceBuffer, IN.UVCoord);
	float4 shaft = tex2D(TESR_RenderedBuffer, IN.UVCoord) * TESR_SecundaRaysData.z * Amount;
	float3 ray = TESR_SecundaRaysRayColor.rgb;//TESR_SunColor.rgb * TESR_SecundaRaysRayColor.rgb;
	shaft.rgb *= (-forward) * ray * saturate(1.0f - ori.rgb);

	float4 color = ori + shaft;
	color.rgb = BlendSoftLight(color.rgb, (ray * TESR_SecundaRaysRayColor.a * Amount + 0.5f));
