// Underwater fullscreen shader for Skyrim Reloaded
...
float4 Water( VSOUT IN ) : COLOR0
{
	...
	if (world_pos.z < 0 && eyepos.z >= 0 && clipping) {
		...
		if (camera_vector.z > 0)
		{
			...
			waterVolColor = lerp(lerp(TESR_WaterDeepColor.rgb, TESR_WaterShallowColor.rgb, 0.6), TESR_FogColor.rgb, 0.5);
			waterVolColor = scattCoeff * waterVolColor.rgb / (extCoeff * (1 - v));			
			waterVolColor *= exp(extCoeff * (1 - v) * eyepos.z * TESR_WaterSettings.y * 0.01) - exp(-extCoeff * (1 - v) * waterfogDist * 0.01);
		}
		else
		{
			waterVolColor = lerp(lerp(TESR_WaterDeepColor.rgb, TESR_WaterShallowColor.rgb, 0.4), TESR_FogColor.rgb, 0.5);
			waterVolColor = scattCoeff * waterVolColor.rgb / (extCoeff * (1 + SinBoverSinA));
			waterVolColor *= exp(extCoeff * (1 + SinBoverSinA) * eyepos.z * TESR_WaterSettings.y * 0.01) - exp(-extCoeff * (1 + SinBoverSinA) * waterfogDist * 0.01);
		}
		
		color.rgb *= exp(-extCoeff * (max(-world_pos.z + eyepos.z * (1 - TESR_WaterSettings.y), 0) + waterfogDist) * 0.01);
		color.rgb += waterVolColor.rgb + 0.1;
	}
	color.rgb += ditherMat[ screenPos.x%4 ][ screenPos.y%4 ] / 255;
	color.a = 1;
    return color;
}
